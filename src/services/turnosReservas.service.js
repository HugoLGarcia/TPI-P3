...
const create = async (turnoReserva) => {
  ...
  const turnoMedicoExiste =
    await turnosReservasRepository.existeTurnoMedico(
      turnoReserva.id_medico,
  ...
  if (obraSocial.es_particular === 0) {
    const descuento =
      valor * (obraSocial.porcentaje_descuento / 100);
  ...
  return await turnosReservasRepository.create(
    turnoReserva
  );
};
...
const marcarAtendido = async (
  ...
  const turno =
    await turnosReservasRepository.getTurnoById(
      id_turno
    );
  ...
  if (turno.id_usuario !== id_usuario) {
    throw new Error(
      "No tiene permisos para modificar este turno"
    );
  }
  ...
  await turnosReservasRepository.marcarAtendido(
    id_turno
  );
